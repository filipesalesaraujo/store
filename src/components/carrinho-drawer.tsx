	...
	return (
		<Drawer>
			<DrawerTrigger asChild>
				<button onClick={handleOpenDrawer} aria-label="Abrir carrinho" className={`transition-colors flex  items-center gap-2 font-bold hover:text-blue-600 p-2 ${botaoDestacado ? ' rounded-md bg-green-500 text-white' : 'text-blue-500'}`}>
					<FaShoppingCart />
					<span className="flex gap-1"><span className='hidden lg:flex'>Carrinho:</span> {totalItens} itens</span>
				</button>
			</DrawerTrigger>
			<DrawerContent>
				<section className='flex justify-center items-center' role="region" aria-label="Carrinho">
					<div className='max-w-[1360px] w-full px-5 flex justify-between flex-wrap'>
						...
						{carrinho.length === 0 ? (
							<p className="text-xl pb-5">Seu carrinho está vazio.</p>
						) : (
							<>
										...
										{carrinho.map((item, index) => (
											<TableRow key={index}>
												...
												<TableCell className="flex gap-2 items-center">
													<Button variant="ghost" onClick={() => handleDecrease(item as Produto)} aria-label="Diminuir quantidade"> - </Button>
													{item.quantidade}
													<Button variant="ghost" onClick={() => handleIncrease(item as Produto)} aria-label="Aumentar quantidade"> + </Button>
												</TableCell>
												<TableCell className="text-right">R$ {item.preco}</TableCell>
												<TableCell className="text-right">
													<Button variant="destructive" onClick={() => handleRemove(item as Produto)} aria-label="Remover produto"> Remover </Button>
												</TableCell>
											</TableRow>
										))}
									</TableBody>
									...
									{carrinho.map((item, index) => (
										<div key={index} className="flex flex-col gap-2">
											...
											<div className="flex gap-2 items-center">
												<Button className="bg-red-500 hover:bg-red-600" onClick={() => handleDecrease(item as Produto)} aria-label="Diminuir quantidade"> - </Button>
												{item.quantidade}
												<Button className="bg-green-500 hover:bg-green-600" onClick={() => handleIncrease(item as Produto)} aria-label="Aumentar quantidade"> + </Button>
											</div>
